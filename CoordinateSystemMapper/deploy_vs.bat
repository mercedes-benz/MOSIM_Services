...
ECHO [33mdeploy_vs.bat[0m at %cd%\deploy_vs.bat Deploying the CoordinateSystemMapper. 
ECHO _______________________________________________________
ECHO.
...
if %ERRORLEVEL% EQU 0 (
  ...
  cmd /c xcopy /S .\CoordinateSystemMapper\bin\Debug\* .\build
    
  ECHO [92mSuccessfully deployed CoordinateSystemMapper[0m
  exit /b 0
) else (
